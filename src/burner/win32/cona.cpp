...
#define DRV(x) _ftprintf(h, _T(#x) _T(" %s\n"),  DriverToName(x))
	...
	VAR(nVidDX9HardFX);
	_ftprintf(h, _T("\n// If non-zero, draw overlay\n"));
	VAR(bVidOverlay);
	_ftprintf(h, _T("\n// If non-zero, draw bigger overlay\n"));
	VAR(bVidBigOverlay);
	_ftprintf(h, _T("\n// Show scores in unranked mode (only for games with detectors)\n"));
	VAR(bVidShowInputs);
	_ftprintf(h, _T("\n// Show overlay debug inputs\n"));
	VAR(bVidUnrankedScores);
	_ftprintf(h, _T("\n// Save overlay data to files in realtime (for Streaming)\n"));
	VAR(bVidSaveOverlayFiles);
	_ftprintf(h, _T("\n// Save chat history\n"));
	VAR(bVidSaveChatHistory);
	_ftprintf(h, _T("\n// Mute chat\n"));
